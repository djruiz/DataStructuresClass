...
public class HuffTree<Key, E> {
    ...
    private void buildHelper(BinNode<Integer, String> node, String builder){
        if(node == null){
            return;
        }
        if(node.getLeft() != null){
            buildHelper(node.getLeft(), builder + "0");
            buildHelper(node.getRight(), builder + "1");
        }
        else{
            codeTable.put(node.getValue(), builder);
        }
    }
